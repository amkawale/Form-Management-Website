        ...
        <tfoot>
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<tr>
                        <td>{$row['oid']}</td>
                        <td>{$row['vgroup_description']}</td>

                        <td>
                            <button class='btn btn-info btn-action' data-bs-toggle='modal' data-bs-target='#exampleModal' onclick='editData({$row['oid']}, \"{$row['vgroup_description']}\")'>Edit</button>
                            <a class='btn btn-primary btn-action' href='#'>Configure</a>
                            <button class='btn btn-danger btn-action' onclick='deleteValidationData({$row['oid']})'>Delete</button>
                        </td>
                    </tr>";
                }
            } else {
