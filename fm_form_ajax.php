...
if (isset($_REQUEST['json_param'])) {
    ...
    if (isset($json_param['project_oid']) && $json_param['project_oid'] != "") {
        $project_oid = $json_param['project_oid'];
        $form_mode = $json_param['form_mode'];
        $ticket_oid = $json_param['ticket_oid'];
    } else {
    ...
    $form_structure = renderTab($json_param['project_oid'], $form_mode);
    echo $form_structure;
}
...
?>
    <div class="container mt-3">
        ...
        <ul class="nav nav-pills" role="tablist" style="<?php echo $tab_head ?>">
            ...
            ?>
                    <li class="nav-item">
                        <a class="nav-link" data-bs-toggle="pill" onclick="loadTab('<?php echo $rec['oid'] ?>')"><?php echo $rec['tab_name'] ?></a>
                    </li>
                <?php
                }
                ?>
                <li class="nav-item">
                    <a class="nav-link" data-bs-toggle="pill"><button class="btn btn-info" data-bs-toggle="modal" data-bs-target="#exampleModal1">
                            <!-- Add Form -->
