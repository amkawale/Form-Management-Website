    ...
    if (mysqli_num_rows($rec_section) > 0) {
        foreach ($rec_section as $row_section) {
            if ($row_section['oid'] == $section_oid) {
                ...
                $section_css = generateObjectCSS($row_section['theme_id'], 'section');
                $section_header_css = "";
                ...
                <div class='configure-panel container-fluid' style='margin-right:0px;margin-top:0px;'> 
                    <button class='btn btn-danger btn-xs' title='Delete Section' onclick='deleteSection(\"{$row_section['oid']}\",\"$panel_oid\")'> <i class='fa fa-trash-o' aria-hidden='true'> </i></button>
                </div>";
                        ...
                        if ($objectHtml != "NA") {
                            ...
                            $row_html .= "<div class='bs5-col section_cell' $drag_object>
                                    ...
                                    <div class='configure-panel container-fluid' style='margin-right:0px;margin-top:0px;'> 
                                            ...
                                            $validation_button
                                        <button class='btn btn-danger btn-xs' title='Remove Mapped Object' onclick='deleteObjectMapping({$row_section['oid']},$row,$col,\"$section_object_mapping_oid\")'> <i class='fa fa-times' aria-hidden='true'> </i></button>
                                    </div>
                                     $objectHtml
                                  </div>";
                        } else {
                            ...
                            $row_html .= "<div class='bs5-col section_cell' style='text-align: center;display: flex;align-items: center;flex-direction: column;' draggable='true' ondragstart='dragStart(\"{$row_section['section_id']}\",$row,$col,\"\",\"\")' ondrop='dragStop(\"{$row_section['section_id']}\",$row,$col,\"\",\"\")' ondragover='allowDrop(event)' id='new_object_id_{$row_section['section_id']}_{$row}_{$col}'>
                                        <div class='configure-panel container-fluid' style='margin-right:0px;margin-top:0px;'> 
                                            ...
                                            <button class='btn btn-primary btn-xs $hidden' title='Map Object'  data-bs-target='#ObjectMappingexampleModal'  onclick='loadObjectMappingModal(\"{$row_section['oid']}\",$row,$col)'> <i class='fa fa-map-marker' aria-hidden='true'></i></button>";
                            if ($hidden) {
        ...
        foreach ($rec_q_object as $object) {
            if ($object['object_id'] == $txn_object_oid) {
                ...
                if ($object_type == 'label') {
                    ...
                    $object_html = "<select class='form-control' $disabled $attributes  style='$object_css' name='" . $object_id . "' id='" . $object_id . "'><option>select </option></select>
                    <input type='hidden' id='hidden_" . $object_id . "' value=''>
                    <div class='subtext' id='subtext_$object_id'></div>";
